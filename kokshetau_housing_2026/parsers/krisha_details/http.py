...
class KrishaDetailsHttpClient:
    ...
    def fetch_html(self, url: str, max_retries: int = 3) -> Optional[str]:
        for attempt in range(1, max_retries + 1):
            try:
                logger.info("Fetching detail url=%s attempt=%s", url, attempt)
                ...
                if response.status_code in (401, 403, 404):
                    logger.error("Detail unavailable status=%s url=%s", response.status_code, url)
                    return None
